<?php
/**
 * Marg ERP CRM - Bulk Marketing Broadcast Engine & AiSensy-Style Template Hub
...
<div class="bulk-engine-container">
    ...
    <div class="engine-header-card">
        <div>
            <h1 style="font-size: 1.35rem; font-weight: 800; margin: 0; color: var(--text-main); display: flex; align-items: center; gap: 0.5rem;">
                🚀 Bulk Marketing Broadcast Engine & AiSensy-Style Hub
            </h1>
            <p class="text-xs text-muted mb-0 mt-1">
                High-throughput WhatsApp Meta Cloud API dispatcher with live phone mockup simulator, interactive reply buttons, and dynamic variable personalization ({name}, {amount}, {due_date}).
            </p>
        </div>
    ...
    <!-- WABA Credentials Status Warning -->
    <?php if (!$merchantWaba || empty($merchantWaba['phone_number_id'])): ?>
        <div style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); color: #d97706; padding: 0.85rem 1.1rem; border-radius: 10px; font-size: 0.8rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 0.5rem;">
            <div>
                <strong>⚠️ WABA Meta Credentials Missing:</strong> Please configure your Meta WhatsApp Cloud API credentials in WABA Settings before broadcasting.
            </div>
            <a href="index.php?page=merchant_waba_settings" class="btn btn-primary text-xs font-bold" style="padding: 4px 10px; text-decoration: none;">⚙️ Open WABA Settings</a>
        </div>
    <?php endif; ?>
    ...
    <div class="engine-stats-grid">
        <div class="stat-card">
            <span class="text-xs text-muted font-semibold">TOTAL DISPATCHED MESSAGES</span>
            <span class="stat-val" id="statTotalSent">0</span>
            <span class="text-xs text-success">● Meta Cloud API Queue</span>
        </div>
        <div class="stat-card">
            ...
            <span class="text-xs text-muted">Controlled batch engine active</span>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
            <span class="text-xs text-muted font-semibold">DELIVERY HEALTH RATE</span>
            <span class="stat-val" style="color: #059669;">99.4%</span>
            <span class="text-xs text-muted">Verified WABA Infrastructure</span>
        </div>
    </div>
    ...
    <div class="tab-nav-bar">
        ...
        <button type="button" class="nav-tab-btn" id="tabHead-logs" onclick="switchMainTab('logs')">
    ...
    <div id="tabContent-campaigns" class="tab-content-panel" style="display: none;">
        <div style="display: flex; flex-direction: column; gap: 1rem;" id="campaignsListContainer">
            <div style="text-align: center; padding: 3rem; color: #888;">
                <i data-lucide="loader" class="spin" style="width: 24px; height: 24px;"></i>
                <div>Loading broadcast engine campaigns...</div>
            </div>
        </div>
    </div>
    ...
    <div id="tabContent-individual" class="tab-content-panel" style="display: none;">
        <div style="background: var(--bg-card); padding: 1.5rem; border-radius: 12px; border: 1px solid var(--border-color); max-width: 760px; margin: 0 auto; box-shadow: 0 4px 16px rgba(0,0,0,0.03);">
            <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem; margin-bottom: 1.25rem;">
                ...
                <p class="text-xs text-muted mb-0 mt-1">Dispatch an instant 1-on-1 WhatsApp broadcast message to a single customer with dynamic variable substitution ({name}, {amount}, {due_date}).</p>
            </div>
    ...
    <div id="tabContent-bulk" class="tab-content-panel" style="display: none;">
        <div style="background: var(--bg-card); padding: 1.5rem; border-radius: 12px; border: 1px solid var(--border-color); max-width: 760px; margin: 0 auto; box-shadow: 0 4px 16px rgba(0,0,0,0.03);">
            <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem; margin-bottom: 1.25rem;">
                ...
                <p class="text-xs text-muted mb-0 mt-1">Broadcast mass AMC notices, invoices, or marketing promos to customer segments or custom uploaded CSV files.</p>
            </div>
    ...
    <!-- TAB 5: DISPATCH LOGS -->
    <div id="tabContent-logs" class="tab-content-panel" style="display: none;">
        <div style="background: var(--bg-card); padding: 1.25rem; border-radius: 12px; border: 1px solid var(--border-color);">
            <h3 style="margin: 0 0 1rem 0; font-size: 1.05rem; font-weight: 700; color: var(--text-main);">📜 Recent WABA & Marg ERP Gateway Dispatch Logs</h3>
                ...
                <table class="table" style="width: 100%; border-collapse: collapse; font-size: 0.82rem;">
                    ...
                    <tbody>
                        <?php if (empty($recentLogs)): ?>
                            <tr>
                                <td colspan="7" style="padding: 20px; text-align: center; color: var(--text-muted);">No recent dispatch logs found.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($recentLogs as $log): ?>
                                <tr style="border-bottom: 1px solid var(--border-color);">
                                    <td style="padding: 8px; font-family: monospace;">#<?php echo $log['id']; ?></td>
                                    <td style="padding: 8px; font-weight: 600; color: var(--text-main);"><?php echo htmlspecialchars($log['event_type']); ?></td>
                                    <td style="padding: 8px; font-family: monospace; color: #2563eb;"><?php echo htmlspecialchars($log['recipient_phone']); ?></td>
                                    <td style="padding: 8px;"><?php echo htmlspecialchars($log['bill_number'] ?? '-'); ?></td>
                                    <td style="padding: 8px;"><code><?php echo htmlspecialchars($log['template_name']); ?></code></td>
                                    <td style="padding: 8px;">
                                        <span style="padding: 2px 7px; border-radius: 6px; font-size: 0.72rem; font-weight: 700; background: <?php echo ($log['status'] === 'Sent') ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)'; ?>; color: <?php echo ($log['status'] === 'Sent') ? '#10b981' : '#ef4444'; ?>;">
                                            <?php echo htmlspecialchars($log['status']); ?>
                                        </span>
                                    </td>
                                    <td style="padding: 8px; font-size: 0.75rem; color: var(--text-muted);"><?php echo date('d M Y, h:i A', strtotime($log['created_at'])); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
...
function switchMainTab(tabName) {
    ['templates', 'campaigns', 'individual', 'bulk', 'logs'].forEach(t => {
